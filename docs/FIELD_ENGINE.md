...
- The renderer and a deployment screen. (Note: **formation is held by the
  `spacing` order, not by initial placement** — units converge within ~1 s.)
- Traits differentiate *targeting* but not yet *movement*; an assassin and an
  anchor still advance similarly.
- Battle sprites — blocked on generation, see `docs/BATTLE_SPRITES.md`.
