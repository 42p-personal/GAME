...
const AVIAN: SignatureDef[] = [
  ...
  def({ name: 'Carrion Omen', stat: 'WIS', type: 'damage', channel: 'support', target: 'enemy', cooldown: 6, accuracy: 92, power: 46, status: { kind: 'doom', chance: 35, duration: 4 }, effects: { spreadStatus: { targets: 2, chance: 45 } }, desc: 'Marks one for the end — and what it carries passes to two more of the flock.' }),
  def({ name: 'Called Home', stat: 'WIS', type: 'buff', channel: 'support', target: 'team', cooldown: 6, accuracy: 100, power: 30, effects: { cleanse: true }, desc: 'The whole flock answers: 30 HP restored to each ally and every ailment cleared.' }),
  def({ name: 'The Long Migration', stat: 'WIS', type: 'buff', channel: 'support', target: 'team', cooldown: 6, accuracy: 100, power: 0, effects: { regenBuff: 4, hpRegenBuff: 5, accBuff: 14, duration: 4 }, desc: 'The flight that sustains them: +4 mana, +5 HP per turn and +14% accuracy, team-wide, for 4 rounds.' }),
]
...
const MARSUPIAL: SignatureDef[] = [
  def({ name: 'The Vanishing Act', stat: 'CHA', type: 'buff', channel: 'voice', target: 'team', cooldown: 5, accuracy: 100, power: 0, effects: { dodgeBuff: 16, accBuff: 14, duration: 4 }, desc: 'A troupe nobody can pin down: +16% dodge and +14% accuracy, team-wide, for 4 rounds.' }),
  // SETTER — pool Screech sets fear at 20%; cashed by pool Siren's Call.
  def({ name: "Barker's Cry", stat: 'CHA', type: 'damage', channel: 'voice', target: 'allEnemies', cooldown: 5, accuracy: 88, power: 42, status: { kind: 'fear', chance: 28, duration: 2 }, desc: 'Works the crowd against them; 28% chance to put fear through the whole enemy line.' }),
  ...
  def({ name: 'The Long Con', stat: 'CHA', type: 'damage', channel: 'voice', target: 'enemy', cooldown: 5, accuracy: 90, power: 56, status: { kind: 'fear', chance: 55, duration: 2 }, effects: { bonusVsStatus: { kind: 'fear', mult: 1.8, consume: true } }, desc: 'Breaks their nerve to cash it in — then puts it right back.' }),
  // SPREADER. Blind is a modifier rather than a hijack, so it can safely carry.
  def({ name: 'Carnival of Errors', stat: 'CHA', type: 'damage', channel: 'voice', target: 'enemy', cooldown: 6, accuracy: 90, power: 46, status: { kind: 'blind', chance: 50, duration: 3 }, effects: { spreadStatus: { targets: 1, chance: 30 } }, desc: 'Misdirection that catches: 50% to blind, and the confusion passes to a neighbour.' }),
  def({ name: 'Ashfall Elegy', stat: 'CHA', type: 'damage', channel: 'voice', target: 'allEnemies', cooldown: 5, accuracy: 88, power: 38, status: { kind: 'healblock', chance: 24, duration: 3 }, desc: 'A dirge for everything the fires took; 24% chance to stop each foe healing.' }),
  def({ name: 'The Grand Parade', stat: 'CHA', type: 'buff', channel: 'voice', target: 'team', cooldown: 6, accuracy: 100, power: 0, status: { kind: 'haste', chance: 100, duration: 2 }, effects: { atkBuff: 0.22, hpRegenBuff: 5, duration: 4 }, desc: 'The whole company marching in step: +22% damage, +5 HP/turn, and the team moves first.' }),
]
...
const INSECTOID: SignatureDef[] = [
  def({ name: 'Chitin Bulwark', stat: 'CON', type: 'buff', channel: 'support', target: 'self', cooldown: 5, accuracy: 100, power: 0, effects: { ward: 42, thorns: 7, defBuff: 6, duration: 4 }, desc: 'Layers on shell: a 42-point shield, 7 thorns and +6 armour for 4 rounds.' }),
  // Spends ward set by pool Bastion (25) or Fortify (40) — or Nautilux's innate.
  def({ name: 'Shatterguard', stat: 'CON', type: 'damage', channel: 'melee', target: 'enemy', cooldown: 5, accuracy: 90, power: 46, effects: { consumeWard: 0.02 }, desc: 'Drives its own shield into the blow — the bigger the shell, the worse the hit.' }),
  // Spends thorns set by pool Barbed Carapace (6).
  def({ name: 'Barbfall', stat: 'CON', type: 'damage', channel: 'melee', target: 'frontRow', cooldown: 5, accuracy: 88, power: 38, effects: { consumeThorns: 0.05 }, desc: 'Rips its barbs free and rakes them across the enemy front line.' }),
  def({ name: 'Unbroken', stat: 'CON', type: 'damage', channel: 'melee', target: 'enemy', cooldown: 5, accuracy: 90, power: 52, status: { kind: 'knockback', chance: 40, duration: 2 }, effects: { hpScale: { atFull: 1.7, atEmpty: 0.6 } }, desc: 'A fortress hits hardest while its walls still stand.' }),
  def({ name: 'The Hive Answers', stat: 'CON', type: 'debuff', channel: 'support', target: 'allEnemies', cooldown: 6, accuracy: 100, power: 0, effects: { tauntForce: true, guard: 23, thorns: 7, duration: 3 }, desc: 'Pulls the entire enemy line onto itself, braced behind 23 guard and 7 thorns.' }),
  def({ name: 'Long Succession', stat: 'CON', type: 'buff', channel: 'support', target: 'self', cooldown: 6, accuracy: 100, power: 60, effects: { cleanse: true, hpRegenBuff: 5, duration: 3 }, desc: 'The hive endures: 60 HP restored, every ailment cleared, +5 HP/turn.' }),
]
...
const REPTILIAN: SignatureDef[] = [
  ...
  def({ name: 'Coil and Strike', stat: 'DEX', type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 5, accuracy: 92, power: 24, status: { kind: 'bleed', chance: 45, duration: 3 }, effects: { hits: [2, 4], randomTargets: true }, desc: 'Two to four strikes from stillness, wherever they land.' }),
  def({ name: 'Sunward Basking', stat: 'DEX', type: 'buff', channel: 'support', target: 'self', cooldown: 5, accuracy: 100, power: 0, status: { kind: 'haste', chance: 100, duration: 2 }, effects: { dodgeBuff: 16, accBuff: 14, duration: 4 }, desc: 'Soaks up the heat and comes alive: +16% dodge, +14% accuracy, and moves first.' }),
  // PAYOFF — pool Deadeye cashes bleed at x1.5.
  def({ name: 'Ambush from Stillness', stat: 'DEX', type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 5, accuracy: 92, power: 50, effects: { bonusVsStatus: { kind: 'bleed', mult: 2.1, consume: true }, execute: 0.3 }, desc: 'The strike it has been waiting the whole fight to make.' }),
  def({ name: 'Tailwhip Sweep', stat: 'DEX', type: 'damage', channel: 'ranged', target: 'frontRow', cooldown: 5, accuracy: 88, power: 40, status: { kind: 'knockback', chance: 46, duration: 2 }, desc: 'Takes the legs from the whole front line; 46% chance to drive each back.' }),
]
...
const PRESTIGE_SHARED: SignatureDef[] = [
  ...
  def({ name: 'Aeons of Patience', stat: 'WIS', type: 'damage', channel: 'support', target: 'enemy', cooldown: 5, accuracy: 92, power: 56, effects: { bonusVsStatus: { kind: 'doom', mult: 2.0, consume: true } }, desc: 'It has waited longer than this fight; doubles down on anything already doomed.' }),
  def({ name: 'Deepwater Hymn', stat: 'WIS', type: 'damage', channel: 'support', target: 'backRow', cooldown: 5, accuracy: 90, power: 46, status: { kind: 'silence', chance: 29, duration: 2 }, effects: { manaBurn: 20 }, desc: 'Sung at the ranks behind the wall; 29% to silence, and burns 20 MP.' }),
  def({ name: 'Wyrmscale Aegis', stat: 'WIS', type: 'buff', channel: 'support', target: 'self', cooldown: 5, accuracy: 100, power: 0, effects: { defBuff: 8, thorns: 7, hpRegenBuff: 5, duration: 4 }, desc: 'Scales that have turned worse than this: +8 armour, 7 thorns, +5 HP/turn.' }),
]
...
const MYTHICAL: SignatureDef[] = [
  def({ name: 'The Unison', stat: 'CHA', type: 'buff', channel: 'voice', target: 'team', cooldown: 6, accuracy: 100, power: 0, status: { kind: 'haste', chance: 100, duration: 2 }, effects: { atkBuff: 0.28, accBuff: 14, duration: 4 }, desc: 'One will across the whole team: +28% damage, +14% accuracy, and everyone moves first.' }),
  // SETTER — pool Screech sets fear at 20%; cashed by pool Siren's Call.
  def({ name: 'Prehistoric Roar', stat: 'CHA', type: 'damage', channel: 'voice', target: 'allEnemies', cooldown: 5, accuracy: 88, power: 46, status: { kind: 'fear', chance: 28, duration: 2 }, desc: 'A sound older than the circuit; 28% chance to break the whole enemy line.' }),
